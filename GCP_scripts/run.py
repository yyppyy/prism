...
# lock_types = ['pthread_rwlock_prefer_w', 'percpu', 'mcs',
#             #   'pthread_mutex'
#               ]
                    ...
                    for num_nodes in num_nodess:
                        for num_threads_per_nodes in num_threads_per_nodess:
                            ...
                            gcp_cmd = ' '.join((bin_file, str(num_nodes), str(num_threads_per_nodes), str(num_locks),
                                        workload, lock_type, mem_addr_file, '0', str(think_time), str(req_interval), str(start_delay),
                                        str(warmup_iters), str(num_iters), str(rmax), str(wmax)))
                            cmd += gcp_cmd
                            print(cmd)
